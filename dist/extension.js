...
var SidebarProvider = class {
  ...
  static viewType = "groqCoder.chatView";
...
var PagePromise = class extends APIPromise {
  constructor(client, request2, Page) {
    super(request2, async (props) => new Page(client, props.response, await defaultParseResponse(props), props.options));
  }
  /**
        ...
        res.on("end", () => {
          try {
            ...
            const models = (json.data ?? []).filter(
              (m2) => !m2.id.includes("whisper") && !m2.id.includes("embed") && !m2.id.includes("guard") && !m2.id.includes("tts")
...
function selectAutoModel(prompt) {
...
function buildSystemPrompt(planMode) {
...
async function handleGroqRequestStreaming(prompt, model = "llama-3.3-70b-versatile", planMode = false, onChunk) {
  const config = vscode2.workspace.getConfiguration("groqCoder");
  const apiKey = config.get("apiKey");
  if (!apiKey) {
    const msg = "\u26A0\uFE0F No API key set. Use the \u{1F511} button in the toolbar to add your Groq API key.";
    onChunk(msg);
    return msg;
  }
  const groq = new groq_sdk_default({ apiKey });
...
${context}` : "");
  ...
  try {
    ...
    const msg = error.message || String(error);
    if (msg.includes("401") || msg.toLowerCase().includes("invalid api key") || msg.toLowerCase().includes("invalid_api_key")) {
      const errMsg2 = "\u274C Invalid API key.\n\nClick the \u{1F511} button in the toolbar above to enter a valid Groq API key.\n\nGet a free key at console.groq.com/keys";
      onChunk(errMsg2);
      return errMsg2;
    }
...
var ChatPanel = class _ChatPanel {
  static current;
  static viewType = "groqCoder.chat";
  _panel;
  _extensionUri;
  _context;
  _disposables = [];
  static createOrShow(extensionUri, extensionContext) {
    if (_ChatPanel.current) {
      _ChatPanel.current._panel.reveal(vscode3.ViewColumn.Beside);
      return;
    }
    ...
    _ChatPanel.current = new _ChatPanel(panel, extensionUri, extensionContext);
  }
  constructor(panel, extensionUri, context) {
    ...
    this._panel.webview.onDidReceiveMessage(async (message) => {
      switch (message.command) {
        case "ready": {
          const history = this._context.globalState.get("groqCoder.chatHistory", []);
        ...
        case "saveApiKey": {
          ...
          await vscode3.workspace.getConfiguration("groqCoder").update("apiKey", key, vscode3.ConfigurationTarget.Global);
          this._panel.webview.postMessage({ command: "apiKeySaved" });
          this._loadModels();
          return;
        }
        case "clearApiKey": {
          await vscode3.workspace.getConfiguration("groqCoder").update("apiKey", "", vscode3.ConfigurationTarget.Global);
          this._panel.webview.postMessage({ command: "apiKeyCleared" });
          return;
        }
        case "openExternal": {
          vscode3.env.openExternal(vscode3.Uri.parse(message.url));
          return;
        }
  ...
  async _loadModels() {
    const apiKey = vscode3.workspace.getConfiguration("groqCoder").get("apiKey") ?? "";
    if (!apiKey.trim()) {
      return;
    }
    try {
  ...
  async _handleFileCreation(response) {
    ...
    for (const regex of [p1, p2, p3]) {
      ...
      while ((match = regex.exec(response)) !== null) {
        ...
        const fileUri = vscode3.Uri.joinPath(workspaceFolder.uri, filename);
        if (filename.includes("/")) {
          const parentPath = filename.split("/").slice(0, -1).join("/");
          await vscode3.workspace.fs.createDirectory(
            vscode3.Uri.joinPath(workspaceFolder.uri, parentPath)
          );
        }
        await vscode3.workspace.fs.writeFile(fileUri, Buffer.from(content, "utf8"));
        createdFiles.push(filename);
      }
    }
    if (createdFiles.length > 0) {
      const cleaned = response.replace(/### FILE: .+?\n```[\w]*\n[\s\S]*?```/g, "").replace(/\*\*(?:[Ff]ile:\s*)?[^\*\n]+\.\w+\*\*\s*\n```[\w]*\n[\s\S]*?```/g, "").replace(/`[^`\n]+\.\w+`\s*\n```[\w]*\n[\s\S]*?```/g, "").trim();
      return {
        text: `${cleaned}

\u2705 Created ${createdFiles.length} file(s) in your workspace:
${createdFiles.map((f2) => `  \u2022 ${f2}`).join("\n")}`,
        files: createdFiles
      };
    }
    return { text: response, files: [] };
  ...
  _getHtml(webview) {
    ...
    const scriptUri = webview.asWebviewUri(
      vscode3.Uri.joinPath(this._extensionUri, "media", "sidebar.js")
    );
    return `<!DOCTYPE html>
<html lang="en">
<head>
...
<div id="onboarding" style="${onboardingStyle}">
    ...
    <div id="setup-bubbles">
        <div class="setup-bubble">\u{1F44B} Hey! Welcome to Groq Coder.</div>
        ...
        <div class="setup-bubble">To get started, paste your free Groq API key below. It takes 30 seconds to get one. \u{1F680}</div>
    </div>
    <div id="key-section">
        <button class="get-key-btn" id="get-key-btn">\u{1F511} Get your free API key at console.groq.com \u2192</button>
        <div class="key-row">
...
<div id="chat-app" style="${chatStyle}">
    <div class="toolbar">
        <select id="model-select" title="Select AI model">
            ...
            <optgroup label="\u2500\u2500 Production \u2500\u2500">
                <option value="llama-3.3-70b-versatile">Llama 3.3 70B \xB7 Best Quality</option>
                <option value="llama-3.1-8b-instant">Llama 3.1 8B \xB7 Fastest</option>
            </optgroup>
            <optgroup label="\u2500\u2500 Preview \u2500\u2500">
                <option value="moonshotai/kimi-k2-instruct-0905">Kimi K2 \xB7 262K context</option>
                <option value="meta-llama/llama-4-scout-17b-16e-instruct">Llama 4 Scout 17B</option>
            </optgroup>
        </select>
        <button class="toolbar-btn" id="plan-btn" title="Toggle Plan Mode">\u{1F4CB} Plan</button>
    ...
    <div class="input-area">
        ...
        <div class="input-footer">
            <span class="hint-text">Enter \u21B5 send \xB7 Shift+Enter newline \xB7 / commands</span>
...
<div id="key-overlay">
    <div class="overlay-card">
        <div class="overlay-title">\u{1F511} Update API Key</div>
...
<script src="${scriptUri}"></script>
  ...
  dispose() {
    ...
    while (this._disposables.length) {
      const d2 = this._disposables.pop();
      if (d2) {
        d2.dispose();
      }
    }
...
function activate(context) {
  ...
  context.subscriptions.push(
    vscode4.commands.registerCommand("groqCoder.startChat", () => {
